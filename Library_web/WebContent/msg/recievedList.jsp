...
<table class="table">
	<caption class="h4">받은쪽지함</caption>
	<thead>
		<tr>
			<th>번호</th>
			<th>제목</th>
	...
	<tbody>
		<c:forEach var="l" items="${list }" varStatus="status">
			<tr>
				...
				<td><a href="${pageContext.request.contextPath }/msgV?idx=${l.idx }">${l.title }</a></td>
				<td>${l.from_nickname }</td>
				<td>${l.sendDate }</td>
				<td>
...
<nav style="text-align: center;">
	<ul class="pagination pagination-sm">
		<c:if test="${page != 1 }">
			<li><a href="${pageContext.request.contextPath }/msg">처음</a></li>
		</c:if>
		<c:if test="${page == 1 }">
		...
		<c:if test="${startPage != 1 }">
			<li><a href="${pageContext.request.contextPath }/msg?page=${startPage - 1}">이전</a></li>
		</c:if>
		<c:if test="${startPage == 1 }">
		...
		<c:forEach var="i" begin="${startPage }" end="${endPage }">
			...
			<c:if test="${i != page }">
				<li><a href="${pageContext.request.contextPath }/msg?page=${i}">${i }</a></li>
			</c:if>
		</c:forEach>
		
		<c:if test="${endPage != totalPage }">
			<li><a href="${pageContext.request.contextPath }/msg?page=${endPage + 1}">다음</a></li>
		</c:if>
		<c:if test="${endPage == totalPage }">
